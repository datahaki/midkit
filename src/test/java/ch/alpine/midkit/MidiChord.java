...
public class MidiChord {
  public final SingleTone[] singleTone = { new SingleTone(0), new SingleTone(1), new SingleTone(2) };
  ...
  static void main() throws Exception {
    ...
    if (!midiDevice.isOpen())
      midiDevice.open();
    // ---
    MidiChord midiOnOff = new MidiChord();
    midiOnOff.singleTone[0].instrumentSelection.midiInstrument = MidiInstrument.GRAND_PIANO;
    midiOnOff.singleTone[1].instrumentSelection.midiInstrument = MidiInstrument.NYLON_GUITAR;
    midiOnOff.singleTone[2].instrumentSelection.midiInstrument = MidiInstrument.VIOLIN;
    ...
    Runnable runnable = new Runnable() {
      ...
      public void run() {
        if (last != midiOnOff.play) {
          last = midiOnOff.play;
          try {
            for (int index = 0; index < midiOnOff.singleTone.length; ++index) {
              SingleTone singleTone = midiOnOff.singleTone[index];
              int channel = singleTone.channel;
              receiver.send(new ShortMessage(ShortMessage.PROGRAM_CHANGE, channel, singleTone.instrumentSelection.midiInstrument.ordinal(), 0), -1); // select
                                                                                                                                                     // piano
              int pitch = singleTone.pitch.intValue();
              int vel = singleTone.vel.intValue();
              if (midiOnOff.play)
