...
public class DrumKitDemo {
  public final DrumKitParam singleTone = new DrumKitParam();
  ...
  static void main() throws Exception {
    ...
    if (!midiDevice.isOpen())
      midiDevice.open();
    // ---
    DrumKitDemo midiOnOff = new DrumKitDemo();
    ...
    Runnable runnable = new Runnable() {
      ...
      public void run() {
        if (last != midiOnOff.play) {
          last = midiOnOff.play;
          try {
            int channel = Midi.DRUMKIT_CHANNEL;
            int pitch = midiOnOff.singleTone.drumKit.pitch();
